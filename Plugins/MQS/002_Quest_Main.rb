...
class Player_Quests
  ...
  def failQuest(quest,color,story)
    ...
    for i in 0...@completed_quests.length
      if @completed_quests[i].id == quest
        pbMessage("You have already completed this task.")
        return
      end
    end
    for i in 0...@failed_quests.length
      if @failed_quests[i].id == quest
        pbMessage("You have already failed this task.")
        return
      end
    end 
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        found = true
        pbMessage(_INTL("\\se[{1}]<ac><c2=#{colorQuest("red")}>Task failed!</c2>\nYour task log has been updated!</ac>",QUEST_FAIL))
        break
      end
    end
    if !found
