...
class MyUI::Scene
  ...
  def animateScene(align = false, smanim = false, &block)
    ...
    Console.echo_li "ARRIVED ON animateScene"
    Console.echo_li "#{@sprites["player"]}"
    Console.echo_li "#{@vector}"
    ...
    @vector.inc = 0.005*(rand(4)+1)
          ...
          self.setVector( 0, 0, 2, 0, 2)
    # update battler sprites
    # @battle.battlers.each_with_index do |b, i|
    #   if b
    #     unless MyUI.get(:smAnim)
    ...
    #     end
    #     if !@orgPos.nil? && @idleTimer > (@lastMotion.nil? ? EliteMyUI::BATTLE_MOTION_TIMER*Graphics.frame_rate : EliteMyUI::BATTLE_MOTION_TIMER*Graphics.frame_rate*0.5) && @vector.finished? && !@safaribattle
    #       @vector.inc = 0.005*(rand(4)+1)
    #       a = MyUI.random_vector(@battle, @lastMotion)
    #       @lastMotion = rand(a.length)
    ...
    #     dmax = (i%2 == 0) ? 4/EliteMyUI::BACK_SPRITE_SCALE : 4; zoomer = (@vector.zoom1**0.75) * zoom * (@sprites["pokemon_#{i}"].dynamax ? dmax : 1)
  ...
  def moveEntireScene(x=0, y=0, lock=true, bypass=false, except=nil)
    # return if !bypass && EliteMyUI::DISABLE_SCENE_MOTION
    for i in 0...4
  ...
  def wait(frames = 1, align = false, &block)
    ...
    frames_to_wait.times do
      ...
      Graphics.update if !MyUI.get(:smAnim)
    end
  end
  #-----------------------------------------------------------------------------
  #  sets scene vector
  #-----------------------------------------------------------------------------
  def setVector(*args)
    # return if EliteMyUI::DISABLE_SCENE_MOTION
    ...
    # vector = MyUI.get_vector(:PLAYER, nil)

    # Console.echo_li "ARRIVED ON setVector #{vector}"
    # x += @orgPos[0] - vector[0]
    # y += @orgPos[1] - vector[1]
